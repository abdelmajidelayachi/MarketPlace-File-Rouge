  ...
  return (
    <Card className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 md:w-1/4 w-full overflow-hidden'>
      <header className={`p-4 ${props.className}`}>
        <h2 className={`font-bold text-2xl ${props.className}`}>{props.title}</h2>
      </header>
      <div className='p-4'>
        <p className="text-gray-700 text-lg">{props.message}</p>
      </div>
      <footer className='p-4 flex justify-end'>
        <button type="button" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" onClick={props.onClick}>Okay</button>
      </footer>
    </Card>
  )
}
